...
@snap[center]
  @size[0.7em](ファイルアクセスにはinode番号を介して,ファイル情報を取得します.</br>)
  @size[0.7em](inode番号は,ファイルのメタデータを内包しています.</br>)
  @size[0.7em](ファイルのinode番号は`ls -i`で調べることができます.</br>)
  @size[0.7em](inode番号は一意なものです.inode番号はファイルに一意に作成されますが,固定されたものではありません.</br>)
  @size[0.7em](ファイルは最終アクセスしたファイルほど通常,番号は大きくなります.</br>)
@snapend
